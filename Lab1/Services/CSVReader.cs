                ...
                using (var csv = new CsvReader(reader, CultureInfo.InvariantCulture))
                {
                    ...
                    while (csv.Read())
                    {
                        ...
                        while (index < csv.Context.Record.Length)
                        {
                            ...
                            {
                                logger.Error($"Student's {csv.Context.Record[0]} mark of {csv.Context.HeaderRecord[index]} isn't integer");
                                throw;
                            }
                            ...
                            {
                                logger.Error($"Student's {csv.Context.Record[0]} mark of {csv.Context.HeaderRecord[index]} can't be converted in integer");
                                throw;
                            }
                            catch (Exception)
                            {
